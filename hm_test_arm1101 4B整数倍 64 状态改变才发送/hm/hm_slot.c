	...
	while(1)
	{
    	...
    	switch(cur_state)
		{
        	case INIT:
				...
				H2L_MAC_frame.res = 0;
				H2L_MAC_frame.slotnum = MAX_SLTS_PCF;
				H2L_MAC_frame.slotlen = SLOT_LEN;
				H2L_MAC_frame.slotlen = htons(H2L_MAC_frame.slotlen);
        	...
        	case WAN:
				...
				while(1)
				{
					...
					if(slot_cache1_flag == 1)
					{
						rval = hm_MAC_frame_rcv_proc2(&slot_cache1, net_i, &sf_count);
						if(rval == 0x00 && self_sf != 1)
						{
					...
					if(slot_cache2_flag == 1)
					{
						rval = hm_MAC_frame_rcv_proc2(&slot_cache2, net_i, &sf_count);
						if(rval == 0x00 && self_sf != 1)
						{
							//EPT(stderr, "case WAN sem_post\n");
							/* 发送勤务帧和LowMAC时隙表 */
				...
				if(neighbor_map_manage[net_i]->sf_rf_get == 1 && neighbor_map_manage[net_i]->sf_lc_get == 1 && neighbor_map_manage[net_i]->sf_lc_flag == 1)      /* 满足CON2_3 */
				{
					...
					H2L_MAC_frame.localID = neighbor_map[net_i]->localID;
					H2L_MAC_frame.lcclock_lv = neighbor_map[net_i]->lcclock_lv;
					H2L_MAC_frame.state = 2;          /* 节点状态变为 NET */
					H2L_MAC_frame.res = 0;
					H2L_MAC_frame.slotnum = MAX_SLTS_PCF;
					H2L_MAC_frame.slotlen = SLOT_LEN;
					H2L_MAC_frame.slotlen = htons(H2L_MAC_frame.slotlen);
			...
			case NET:
				...
				while(1)
				{
					...
					{	
						...
						rval = hm_MAC_frame_rcv_proc3(&slot_cache1, net_i, &sf_count);
						if(rval == 0x00 && self_sf != 1)
						{
					...
					if(slot_cache2_flag == 1)
					{
						rval = hm_MAC_frame_rcv_proc3(&slot_cache2, net_i, &sf_count);
						if(rval == 0x00 && self_sf != 1)
						{
				...
				if(neighbor_map[net_i]->netID != neighbor_map[net_i]->localID && neighbor_map_manage[net_i]->sf_samenet_get == 1 && neighbor_map_manage[net_i]->sf_rf_num == 0)    /* 满足CON3_7 CON3_8 CON3_9 */
				{
					int count = 0;
					int flag = 0;
					...
					/* 判断有没有大于等于上级节点时钟级别的勤务帧,以及两跳范围内有没有小于自己节点号的同级节点 */
					/* 此时上级节点应被判断为退网，所以在时隙表里应该看不到上级节点，有待后续修改 11.08 */
					for(i=0; i<MAX_CFS_PSF; i++)
					{
						if(neighbor_map[net_i]->BS[i].BS_flag == 0)
							continue;
						if(neighbor_map[net_i]->BS[i].BS_ID == neighbor_map[net_i]->referenceID)
						{
							count++;
							continue;
						}
						if(neighbor_map[net_i]->BS[i].clock_lv >= neighbor_map[net_i]->rfclock_lv && neighbor_map[net_i]->BS[i].hop == 1 && neighbor_map[net_i]->BS[i].state == NET)
						{
							flag = 1;      /* 有大于等于上级节点时钟级别的勤务帧 */
							break;
						}
						else
						{
							if(neighbor_map[net_i]->BS[i].BS_ID < neighbor_map[net_i]->localID && neighbor_map[net_i]->BS[i].clock_lv == neighbor_map[net_i]->lcclock_lv)
							{
								flag = 2;      /* 两跳范围内有小于自己节点号的同级节点 */
								break;
							}
							else
								count++;
						...
						if(count == neighbor_map[net_i]->BS_num)
							break;
					}
					...
					if(flag == 1)        /* 满足CON3_7 */
					{
						...
						neighbor_map[net_i]->localID = 0;
						neighbor_map[net_i]->lcclock_lv = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].BS_ID = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].BS_flag = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].clock_lv = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].state = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].hop = 0;
						neighbor_map[net_i]->BS[neighbor_map[net_i]->l_BS].life = 0;
						neighbor_map[net_i]->l_BS = 0;
						neighbor_map[net_i]->BS_num--;
						/* 进一步清空在后面 */						
						...
						for(i=0; i<MAX_CFS_PSF; i++)
						{
							if(neighbor_map[net_i]->BS[i].BS_flag == 0 || neighbor_map[net_i]->BS[i].BS_ID == neighbor_map[net_i]->referenceID || neighbor_map[net_i]->BS[i].hop != 1 || neighbor_map[net_i]->BS[i].state != NET)
								continue;
							if(level > neighbor_map[net_i]->BS[i].clock_lv)
							{
								level = neighbor_map[net_i]->BS[i].clock_lv;
								bs_i = i;    /* 上级节点时隙号确定 */
							}
						}
						neighbor_map[net_i]->referenceID = neighbor_map[net_i]->BS[bs_i].BS_ID;
						neighbor_map[net_i]->rfclock_lv = neighbor_map[net_i]->BS[bs_i].clock_lv;
						neighbor_map[net_i]->r_BS = bs_i;

						/*********** 选定BS，更新邻节点表维护表 ***********/
						for(i=0; i<MAX_CFS_PSF; i++)
						{
							if(neighbor_map[net_i]->BS[i].BS_flag == 1)
								continue;
							break;
						}
						/* 此时的i即为本节点选定的时隙 */
						neighbor_map[net_i]->BS_flag = 1;
						neighbor_map[net_i]->localID = localID;
						neighbor_map[net_i]->lcclock_lv = neighbor_map[net_i]->BS[neighbor_map[net_i]->r_BS].clock_lv+1;
						neighbor_map[net_i]->l_BS = i;
						neighbor_map[net_i]->BS_num++;

						neighbor_map[net_i]->BS[i].BS_ID = localID;
						neighbor_map[net_i]->BS[i].BS_flag = 1;
						neighbor_map[net_i]->BS[i].clock_lv = neighbor_map[net_i]->lcclock_lv;
						neighbor_map[net_i]->BS[i].state = WAN;    /* 改变时隙表中的节点状态 */
						neighbor_map[net_i]->BS[i].hop = 0;
						neighbor_map[net_i]->BS[i].life = 0;
						...
						for(i=0; i<MAX_CFS_PSF; i++)
						{
							if(neighbor_map[net_i]->BS[i].BS_flag == 1 && neighbor_map[net_i]->BS[i].hop != 2)
							{
								service_frame.BS[i].BS_ID = neighbor_map[net_i]->BS[i].BS_ID;
								service_frame.BS[i].flag = 1;
								service_frame.BS[i].clock_lv = neighbor_map[net_i]->BS[i].clock_lv;
								service_frame.BS[i].state = neighbor_map[net_i]->BS[i].state;
								max = i;
							}
						}
						service_frame.num = max+1;
					...
					if(flag == 2)     /* 满足CON3_8 */
					{
						...
						for(i=0; i<netnum; i++)
						{
							...
							for(bs=0; bs<MAX_CFS_PSF; bs++)
							{
								if(neighbor_map[i]->BS[bs].BS_flag != 1 || neighbor_map[i]->BS[bs].life == 0)
									continue;
								/* 清空时隙生存期维护线程 11.01*/
								rval = pthread_cancel(neighbor_map_manage[i]->bs_timer[bs]);
								if(rval != 0)						
									EPT(stderr, "CON3_8: pthread_cancel error code1 %d\n", rval);
								rval = pthread_join(neighbor_map_manage[i]->bs_timer[bs], &thread_result);
								if(rval != 0)						
									EPT(stderr, "CON3_8: pthread_join error code1 %d\n", rval); 						
								
								rval = pthread_cancel(neighbor_map_manage[i]->bs_id[bs]);
								if(rval != 0)						
									EPT(stderr, "CON3_8: pthread_cancel error code2 %d\n", rval);
								rval = pthread_join(neighbor_map_manage[i]->bs_id[bs], &thread_result);
								if(rval != 0)						
									EPT(stderr, "CON3_8: pthread_join error code2 %d\n", rval); 
								
								sem_destroy(&(neighbor_map_manage[i]->bs_sem[bs]));	
							}
					...
					/* 两跳范围内有小于自己节点号的同级节点,即自己是节点号最小的同级节点 */
					else      /* 满足CON3_9 */
					{
						EPT(stderr, "3_9\n");
						/*********** 清空操作 ***********/
						...
						neighbor_map[netnum]->BS[0].BS_ID = localID;
						neighbor_map[netnum]->BS[0].BS_flag = 1;
						neighbor_map[netnum]->BS[0].clock_lv = 0;
						neighbor_map[netnum]->BS[0].state = NET;       /* 改变时隙表中的节点状态 */
						neighbor_map[netnum]->BS[0].hop = 0;
						neighbor_map[netnum]->BS[0].life = 0;
						...
						for(i=0; i<MAX_CFS_PSF; i++)
						{
							if(neighbor_map[netnum]->BS[i].BS_flag == 1 && neighbor_map[netnum]->BS[i].hop != 2)
							{
								service_frame.BS[i].BS_ID = neighbor_map[netnum]->BS[i].BS_ID;
								service_frame.BS[i].flag = 1;
								service_frame.BS[i].clock_lv = neighbor_map[netnum]->BS[i].clock_lv;
								service_frame.BS[i].state = neighbor_map[netnum]->BS[i].state;
								max = i;
							}
						}
						service_frame.num = max+1;
...
void hm_MAC_frame_rcv_proc1(lm_packet_t *cache_p, U8 *sf_count)
{
	...
	L2H_MAC_frame_t *L2H_MAC_frame_p = (L2H_MAC_frame_t *)cache_p->data;
	//EPT(stderr, "hm_MAC_frame_rcv_proc1: cache_p->type = %d\n", cache_p->type);	

	switch(cache_p->type)
	{
		case 0:    /* 收到的是勤务帧 */			
			*sf_count = 1;  /* 收到勤务帧置1 11.20 */
			...
			EPT(stderr, "hm_MAC_frame_rcv_proc1: service_frame_p->netID = %d referenceID = %d rfclock_lv = %d r_BS = %d localID = %d lcclock_lv = %d l_BS = %d num = %d\n", 
				service_frame_p->netID, service_frame_p->referenceID, service_frame_p->rfclock_lv, service_frame_p->r_BS, service_frame_p->localID, service_frame_p->lcclock_lv, service_frame_p->l_BS, service_frame_p->num);
			/*
			...
			*/
			...
			for(i=0; i<service_frame_p->num; i++)
			{
				...
				EPT(stderr, "hm_MAC_frame_rcv_proc1: service_frame_p->BS[%d].flag = %d BS_ID = %d state = %d clock_lv = %d\n", i, service_frame_p->BS[i].flag, service_frame_p->BS[i].BS_ID, service_frame_p->BS[i].state, service_frame_p->BS[i].clock_lv);
				/*
		...
		default:
			EPT(stderr, "hm_MAC_frame_rcv_proc1: unknown MAC data, type = %d\n", cache_p->type);
			break;
	}
}

U8 hm_MAC_frame_rcv_proc2(lm_packet_t *cache_p, U8 net_i, U8 *sf_count)
{
	U8   id;        /* for  id = service_frame_p->netID*/
	U8   i;         /* for  for(i=0; i<netnum; i++)   记录时隙号 */
	U8   net_id;   /* 记录当前网络号!!!!! */
	U8   node;     /* 记录占用某时隙的节点号 11.06 */	
	self_sf = 0;	/* 记录是否收到自己发送的勤务帧 */
	...
	{
		case 0:    /* 收到的是勤务帧 */			
			/* 如果收到的勤务帧是自己发的勤务帧，则不处理 11.20 */
			if(service_frame_p->localID == localID)
			{
				self_sf = 1;
				break;
			}
			...
			*sf_count = 1;  /* 收到勤务帧置1 11.20 */
			...
			//EPT(stderr, "hm_MAC_frame_rcv_proc2: service_frame_p->netID = %d referenceID = %d rfclock_lv = %d r_BS = %d localID = %d lcclock_lv = %d l_BS = %d num = %d\n", 
				//service_frame_p->netID, service_frame_p->referenceID, service_frame_p->rfclock_lv, service_frame_p->r_BS, service_frame_p->localID, service_frame_p->lcclock_lv, service_frame_p->l_BS, service_frame_p->num);
			...
			if(net_id == net_i)     /* 表明收到的勤务帧属于本网络 */
			{
				if(service_frame_p->localID == neighbor_map_p->referenceID)    /* 表明收到上级节点勤务帧 */
				{
					...
					for(i=0; i<service_frame_p->num; i++)
					{
						...
						EPT(stderr, "hm_MAC_frame_rcv_proc2: service_frame_p->BS[%d].flag = %d BS_ID = %d state = %d clock_lv = %d\n", i, service_frame_p->BS[i].flag, service_frame_p->BS[i].BS_ID, service_frame_p->BS[i].state, service_frame_p->BS[i].clock_lv);
		...
		default:
			EPT(stderr, "hm_MAC_frame_rcv_proc2: unknown MAC data, type = %d\n", cache_p->type);
			break;
	}
...
U8 hm_MAC_frame_rcv_proc3(lm_packet_t *cache_p, U8 net_i, U8 *sf_count)
{
	U8	 id;		/* for	id = service_frame_p->netID*/
	U8	 i; 		/* for	for(i=0; i<netnum; i++)   记录时隙号 */
	U8	 net_id;   /* 记录当前网络号!!!!! */
	...
	switch(cache_p->type)
	{
		case 0:	  /* 收到的是勤务帧 */			
			...
			*sf_count = 1;  /* 收到勤务帧置1 11.20 */
			...
			//EPT(stderr, "hm_MAC_frame_rcv_proc3: service_frame_p->netID = %d referenceID = %d rfclock_lv = %d r_BS = %d localID = %d lcclock_lv = %d l_BS = %d num = %d\n", 
				//service_frame_p->netID, service_frame_p->referenceID, service_frame_p->rfclock_lv, service_frame_p->r_BS, service_frame_p->localID, service_frame_p->lcclock_lv, service_frame_p->l_BS, service_frame_p->num);
			...
			if(net_id == net_i) 	/* 表明收到的勤务帧属于本网络 */
			{
				...
				if(service_frame_p->localID == neighbor_map_p->referenceID)    /* 表明收到上级节点勤务帧 */
				{
					...
					for(i=0; i<service_frame_p->num; i++)
					{
						...
						EPT(stderr, "hm_MAC_frame_rcv_proc3: service_frame_p->BS[%d].flag = %d BS_ID = %d state = %d clock_lv = %d\n", i, service_frame_p->BS[i].flag, service_frame_p->BS[i].BS_ID, service_frame_p->BS[i].state, service_frame_p->BS[i].clock_lv);
		...
		default:
			EPT(stderr, "hm_MAC_frame_rcv_proc3: unknown MAC data, type = %d\n", cache_p->type);
			break;
	}
...
void hm_LowMAC_slot_proc(U8 netnum)
{
	...
	LM_neighbor_map.slotnum = MAX_SLTS_PCF; 
	LM_neighbor_map.slotlen = SLOT_LEN;
	LM_neighbor_map.slotlen = htons(LM_neighbor_map.slotlen);
...
void *hm_pf_slot_thread(void *arg)
{
	...
	while(1)
	{
		...
		/*EPT(fp, "neighbor_map[%d]->netID = %d  BS_flag = %d  referenceID = %d  rfclock_lv = %d\n r_BS = %d  localID = %d  lcclock_lv = %d  l_BS = %d  BS_num = %d\n", 
