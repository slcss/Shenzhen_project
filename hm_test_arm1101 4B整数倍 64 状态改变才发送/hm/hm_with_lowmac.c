...
void *hm_sendto_lm_thread(void *arg)
{
	...
	{		
		...
		{
			pthread_mutex_lock(&mutex_queue[4]);
			rval = hm_queue_delete(link_queue, 4, &package);
			pthread_mutex_unlock(&mutex_queue[4]);
			if(rval == 1)
				continue;
			...
			EPT(stderr, "hm_sendto_lm_thread: q4 send\n");
			continue;
		}
		...
		{
			pthread_mutex_lock(&mutex_queue[0]);
			rval = hm_queue_delete(link_queue, 0, &package);   /* package.len 是 package.data 里有效数据的长度 */
			pthread_mutex_unlock(&mutex_queue[0]);
			if(rval == 1)
				continue;
		...
		{
			pthread_mutex_lock(&mutex_queue[5]);
			rval = hm_queue_delete(link_queue, 5, &package);   /* package.len 是 package.data 里有效数据的长度 */
			pthread_mutex_unlock(&mutex_queue[5]);
			if(rval == 1)
				continue;
		...
		{			
			if(count == 0)
			{
				...
				count = 1;
				EPT(stderr, "hm_sendto_lm_thread: q05 send3_1\n");
				continue;
			}
			else
			{
				...
				count = 0;
				EPT(stderr, "hm_sendto_lm_thread: q05 send3_2\n");
				continue;
			}
		}
		...
		{			
			pthread_mutex_lock(&mutex_queue[1]);
			rval = hm_queue_delete(link_queue, 1, &package);
			pthread_mutex_unlock(&mutex_queue[1]);
			if(rval == 1)
				continue;
			...
			EPT(stderr, "hm_sendto_lm_thread: q1 send\n");
			continue;
		}
		...
		{			
			pthread_mutex_lock(&mutex_queue[2]);
			rval = hm_queue_delete(link_queue, 2, &package);
			pthread_mutex_unlock(&mutex_queue[2]);
			if(rval == 1)
				continue;
			...
			EPT(stderr, "hm_sendto_lm_thread: q2 send\n");
			continue;
		}
		...
		{			
			pthread_mutex_lock(&mutex_queue[3]);
			rval = hm_queue_delete(link_queue, 3, &package);
			pthread_mutex_unlock(&mutex_queue[3]);
			if(rval == 1)
				continue;
			...
			EPT(stderr, "hm_sendto_lm_thread: q3 send\n");
			continue;
		}
		else
		{
