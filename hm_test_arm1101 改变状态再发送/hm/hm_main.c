...
void* hm_lm2nl_thread(void *arg)
{
	...
	while(0 == stop)
	{
		...
		switch(package->type & 0x0f)
		{
					...
					case 0:     
						switch(((mac_packet_t *)package->data)->subt)
						{
							case 0:  /* 路由协议(其中包含组播数据) */
								...
								if(type == MMSG_MAODV)   /* 是组播数据 */
								{
									rval = hm_smsg_proc(&tx_msg, MMSG_MP_DATA, ((mac_packet_t *)package->data)->src, package->len-4, package->data);  /* package.len-4 为data的真实长度 */
									if(rval != 0)
										goto thread_return;									
								}
								else
			...
			case LH_SF_DATA:     /* 勤务帧反馈   暂时考虑两个缓存区 */
