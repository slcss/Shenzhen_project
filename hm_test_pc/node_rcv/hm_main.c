...
void* hm_lm2nl_thread(void *arg)
{
	...
	while(0 == stop)
	{
		...
		switch(package.type)
		{
			...
			case 0X0B: /* 收到hmtest发来的业务时隙达到通知帧 */
				break;
...
{	
	...
	*(int *)tx_msg.data = mac_qid;
	...
	rval = pthread_create(&rcvfr_nl_tid, NULL, hm_rcvfrom_nl_thread, &(qinfs[re_qin].qid));    /* re_qin为本进程的消息队列标号 */
	if (rval != 0) {
	...
	if (rval != 0) 	{
		EPT(stderr, "%s: can not create dslot thread\n", argv[0]);
		rval = 7;
		goto process_return;
	}
