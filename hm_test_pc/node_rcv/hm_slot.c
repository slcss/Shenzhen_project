...
extern pthread_mutex_t for_data_come;  //为data_come增加互斥量 16.12.13
...
neighbor_map_t *neighbor_map[MAX_NET_NUM];  /* 暂时设定网络数最大为32个 11.05 */
U8  netID[MAX_NET_NUM];      /* 记录现存网络号对应的网络零级节点，暂时设定网络数最大为32个 11.05 */
U8  netnum = 0;     /* 记录现存网络个数 */

/* 邻节点跳数记录表 */
U8 hop_table[MAX_NODE_CNT+1];  //hop_table[i]=0，节点i不在两跳范围内；hop_table[i]=1，节点i为1跳；hop_table[i]=2，节点i为2跳；hop_table[i]=0xff，节点i为本地节点；
								

/* 时隙表管理表结构体指针数组 */
neighbor_map_manage_t *neighbor_map_manage[MAX_NET_NUM];   /* 每一个网络的时隙表对应一个时隙表管理表，暂时设定网络数最大为32个 11.05 */
...
void *hm_slot_thread(void *arg)
{
	...
	U8  level = 32;
	U8  max =0;
	U8  hop1_temp = 0;       /* 记录勤务帧中一跳邻节点的个数 */
	U8  sf_dslot_count = 0;  //勤务帧中动态时隙缓存大小
	U8  sf_dslot_map[100];   //勤务帧中动态时隙表
	U8  sf_dslot_num = 0;    //生成勤务帧动态时隙时计数

	int rval;
	pthread_t sf_ls_send = -1;
    	...
    	switch(cur_state)
		{
        	...
        	case SCN:
				...
				while(1)
				{
					...
					{
						if(netID[i] != 0 && neighbor_map_manage[i]->sf_flag == 1)
						{
					...
					if(net_num == netnum)
					{
						/* 停止定时器 */
						nl_tsch.tmask = nl_tsch.tmask | 1;
						nl_tsch.procs[0].wait = nl_tsch.procs[0].period;                    
				...
				if(sf_count == 0)          /* 满足CON1_1   默认net_num为0 */
				{
					...
					memset(sf_dslot_map, 0, sizeof(sf_dslot_map));
					sf_dslot_count = 0;
						...
						if(neighbor_map[netnum]->BS[i][0].BS_ID != 0)
						{
							...
							{					
								if(neighbor_map[netnum]->BS[i][j].BS_flag)
								{
									...
									{
										if(neighbor_map[netnum]->BS[i][j].hop == 1)
											hop1_temp++;
										...
										/* 生成动态时隙信息，携带非两跳节点的动态时隙信息 */
										if(dslot_map1[0][j] != 0)
										{
											sf_dslot_map[sf_dslot_count++] = j;  //记录节点
											sf_dslot_map[sf_dslot_count++] = dslot_map1[0][j];  //记录节点占用几个动态时隙
											sf_dslot_num = 0;
											for(k = 1; k <= MAX_DSLS_PCF; k++)
											{
												if(dslot_map1[k][j] == 2)  //查找到节点占用的动态时隙，2表示此时隙已被占用
												{
													sf_dslot_map[sf_dslot_count++] = k;  //记录节点占用的动态时隙
													sf_dslot_num++;
													if(sf_dslot_num == dslot_map1[0][j])
														break;
												}
											}
										}
										break;  //一个时隙只能被一个非两跳节点占用，若找到即可退出
									}
								}
					...
					memcpy(mac_packet.data+sizeof(service_frame), sf_dslot_map, sf_dslot_count);
					...
					hm_queue_enter(link_queue, 5, 8+sizeof(service_frame)+sf_dslot_count, (char *)&mac_packet, HL_SF_DATA);     /* 送到 TQ_5 队列 */
					sem_post(&(empty[0]));
				...
				if(net_num == netnum)            /* 满足CON1_2 记得清0操作 */
				{
					...
					for(i=0; i<MAX_NET_NUM; i++)
					{
						if(netID[i] != 0 && num < neighbor_map[i]->BS_num)
						{
						...
						if(neighbor_map[net_i]->BS[i][0].BS_ID != 0)
						{
							...
							{					
								if(neighbor_map[net_i]->BS[i][j].BS_flag)
								{
									...
									{
										if(neighbor_map[net_i]->BS[i][j].hop == 1)
											hop1_temp++;
										...
										if(dslot_map1[0][j] != 0)
										{
											...
											for(k = 1; k <= MAX_DSLS_PCF; k++)
											{
												if(dslot_map1[k][j] == 2)  //查找到节点占用的动态时隙
												{
													sf_dslot_map[sf_dslot_count++] = k;  //记录节点占用的动态时隙
													sf_dslot_num++;
													if(sf_dslot_num == dslot_map1[0][j])
					...
					pthread_mutex_lock(&mutex_queue[5]);
					hm_queue_enter(link_queue, 5, 8+sizeof(service_frame)+sf_dslot_count, (char *)&mac_packet, HL_SF_DATA); 
					sem_post(&(empty[0]));
					pthread_mutex_unlock(&mutex_queue[5]);
					pthread_mutex_lock(&for_data_come);
					data_come++;
				...
				if(net_num < netnum && net_num != 0)   /* 满足CON1_3 */
				{
					...
					for(i=0; i<MAX_NET_NUM; i++)
					{
						if(netID[i] != 0 && neighbor_map_manage[i]->sf_flag == 1)     /* 只选择加入满足条件(收到同一个NET节点两次勤务帧)的网络 */
						{
            	...
            	else           /* 满足CON1_4 */
				{
					...
					net_num = 0;
					sf_count = 0;
					memset(dslot_map1, 0, sizeof(dslot_map1));
					memset(dslot_map2, 0, sizeof(dslot_map2));
        	...
        	case WAN:
				...
				while(1)
				{
					...
					if(timer_flag == 1)    /* 如果是定时器到来的话直接退出 */
					{
						EPT(stderr, "hm_slot_thread: timer2 up\n");
						timer_flag = 0;
						break;
					}
					...
					if(neighbor_map_manage[net_i]->sf_rf_get == 1 && neighbor_map_manage[net_i]->sf_lc_get == 1 && neighbor_map_manage[net_i]->sf_lc_flag == 1)
					{
						/* 停止定时器 */
						nl_tsch.tmask = nl_tsch.tmask | (1<<1);
						nl_tsch.procs[1].wait = nl_tsch.procs[1].period;
			...
			case NET:
				...
				while(1)
				{
					...
					if(timer_flag == 1)    /* 如果是定时器到来的话直接退出 */
					{
						EPT(stderr, "hm_slot_thread: timer3 up\n");
						timer_flag = 0;
						break;
					}
					...
					{						
						nl_tsch.tmask = nl_tsch.tmask | (1<<2);
						nl_tsch.procs[2].wait = nl_tsch.procs[2].period;
...
U8 hm_MAC_frame_rcv_proc2(lm_packet_t *cache_p, U8 net_i)  /* 参数net_i是本地节点加入的网络号 1.12 */
{
	...
	neighbor_map_t *neighbor_map_p;
	service_frame_t *service_frame_p = (service_frame_t *)((mac_packet_t *)cache_p->data)->data;
	char *dslot_p = ((mac_packet_t *)cache_p->data)->data + sizeof(service_frame_t);
				...
				if(service_frame_p->localID == neighbor_map_p->referenceID)    /* 表明收到上级节点勤务帧 */
				{
					...
					for(i=0; i<service_frame_p->num; i++)
					{
						...
						else
						{
							...
							{
								neighbor_map_p->BS[i][node].hop = 1;  //其他节点携带着一跳邻节点的信息，不能作为此一跳邻节点更新的依据，不然就算此节点退网，此节点的信息依旧会在网间传递
								hop_table[node] = 1;
								//sem_post(&(neighbor_map_manage[net_id]->bs_sem[i]));	 /* 此时隙更新完成，发出信号 */
							}
						}
					...
					/* 更新动态时隙表 */
					i = 0;
					while(i < cache_p->len-4-8-sizeof(service_frame_t))					
					{
						for(j = 0; j < dslot_p[i+1]; j++)
						{	
							/* 只更新勤务帧发送节点和本节点两跳节点的动态时隙信息 */
							if(hop_table[dslot_p[i]] == 1 && dslot_p[i] == service_frame_p->localID)
							{	
								EPT(stderr, "proc2_1: hop1 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */	
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].pf = ds_sf_timer;								
								dslot_timer.tmap[dslot_p[i+2+j]] = dslot_timer.tmap[dslot_p[i+2+j]] | (1<<(dslot_p[i]-1));
								if(((dslot_timer.tmask[dslot_p[i+2+j]]>>(dslot_p[i]-1)) & 1) != 0)  //说明定时器处于关闭状态
								{
									dslot_timer.tmask[dslot_p[i+2+j]] = dslot_timer.tmask[dslot_p[i+2+j]]^(1<<(dslot_p[i]-1));
									dslot_timer.procs[dslot_p[i+2+j]][0].wait++;
									EPT(stderr, "proc2_1: dslot_timer.procs[%d][0].wait = %d\n", dslot_p[i+2+j], dslot_timer.procs[dslot_p[i+2+j]][0].wait);
								}
								if(dslot_map1[dslot_p[i+2+j]][dslot_p[i]] != 2)  
								{									
									/* 将未占用或欲占用的时隙标记为已占用 */
									dslot_map1[dslot_p[i+2+j]][dslot_p[i]] = 2;
									dslot_map1[dslot_p[i+2+j]][0]++;
									dslot_map1[0][dslot_p[i]]++;
								}
							}
							else if(hop_table[dslot_p[i]] == 2)
							{
								EPT(stderr, "proc2_1: hop2 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
								...
								if(dslot_map2[dslot_p[i+2+j]][dslot_p[i]] != 2)  
								{									
									/* 将未占用或欲占用的时隙标记为已占用 */
									dslot_map2[dslot_p[i+2+j]][dslot_p[i]] = 2;
									dslot_map2[dslot_p[i+2+j]][0]++;
									dslot_map2[0][dslot_p[i]]++;									
								}
							}
							/* 本地节点占用的时隙不需要从勤务帧中来更新 */
							else if(hop_table[dslot_p[i]] == 0 && dslot_p[i] != localID)
							{
								EPT(stderr, "proc2: unexpected node %d\n", dslot_p[i]);
							}
						}
						i = i+2+dslot_p[i+1];
					}					
				...
				{					
					...
					while(i < cache_p->len-4-8-sizeof(service_frame_t))					
					{
						for(j = 0; j < dslot_p[i+1]; j++)
						{
							...
							if(hop_table[dslot_p[i]] == 1 && dslot_p[i] == service_frame_p->localID)
							{	
								EPT(stderr, "proc2_2: hop1 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */	
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
								...
								if(((dslot_timer.tmask[dslot_p[i+2+j]]>>(dslot_p[i]-1)) & 1) != 0)  //说明定时器处于关闭状态
								{
									dslot_timer.tmask[dslot_p[i+2+j]] = dslot_timer.tmask[dslot_p[i+2+j]]^(1<<(dslot_p[i]-1));
									dslot_timer.procs[dslot_p[i+2+j]][0].wait++;
									EPT(stderr, "proc2_2: dslot_timer.procs[%d][0].wait = %d\n", dslot_p[i+2+j], dslot_timer.procs[dslot_p[i+2+j]][0].wait);
								}		
							...
							{
								EPT(stderr, "proc2_2: hop2 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
...
{
	U8	 id;		/* for	id = service_frame_p->netID*/
	U8	 i,j; 		/* for	for(i=0; i<netnum; i++)   记录时隙号 */
	U8	 net_id;   /* 记录当前勤务帧所在的网络号!!!!! */
	U8   node;     /* 记录占用某时隙的节点号 11.06 */	
	int  res;
	U8   net_count = 0;  /* 选择网络时计数 1.11 */
				...
				if(service_frame_p->localID == neighbor_map_p->referenceID)    /* 表明收到上级节点发送的勤务帧 */
				{
					...
					while(i < cache_p->len-4-8-sizeof(service_frame_t))					
					{
						for(j = 0; j < dslot_p[i+1]; j++)
						{
							...
							if(hop_table[dslot_p[i]] == 1 && dslot_p[i] == service_frame_p->localID)
							{	
								EPT(stderr, "proc3_1: hop1 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */	
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
								...
								if(((dslot_timer.tmask[dslot_p[i+2+j]]>>(dslot_p[i]-1)) & 1) != 0)	//说明定时器处于关闭状态
								{
									dslot_timer.tmask[dslot_p[i+2+j]] = dslot_timer.tmask[dslot_p[i+2+j]]^(1<<(dslot_p[i]-1));
									dslot_timer.procs[dslot_p[i+2+j]][0].wait++;
									EPT(stderr, "proc3_1: dslot_timer.procs[%d][0].wait = %d\n", dslot_p[i+2+j], dslot_timer.procs[dslot_p[i+2+j]][0].wait);
								}
								
								if(dslot_map1[dslot_p[i+2+j]][dslot_p[i]] != 2)  
								{	
									EPT(stderr, "proc3_1: hop1 = 2 \n");
									/* 将未占用或欲占用的时隙标记为已占用 */
									dslot_map1[dslot_p[i+2+j]][dslot_p[i]] = 2;
									dslot_map1[dslot_p[i+2+j]][0]++;
									dslot_map1[0][dslot_p[i]]++;
							...
							{
								EPT(stderr, "proc3_1: hop2 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
							...
							{
								EPT(stderr, "proc3: unexpected node %d\n", dslot_p[i]);
							}
						}
						i = i+2+dslot_p[i+1];
					}		
				...
				else
				{
					...
					while(i < cache_p->len-4-8-sizeof(service_frame_t))					
					{
						for(j = 0; j < dslot_p[i+1]; j++)
						{
							...
							if(hop_table[dslot_p[i]] == 1 && dslot_p[i] == service_frame_p->localID)
							{	
								EPT(stderr, "proc3_2: hop1 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */	
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
								...
								if(((dslot_timer.tmask[dslot_p[i+2+j]]>>(dslot_p[i]-1)) & 1) != 0)	//说明定时器处于关闭状态
								{
									dslot_timer.tmask[dslot_p[i+2+j]] = dslot_timer.tmask[dslot_p[i+2+j]]^(1<<(dslot_p[i]-1));
									dslot_timer.procs[dslot_p[i+2+j]][0].wait++;
									EPT(stderr, "proc3_2: dslot_timer.procs[%d][0].wait = %d\n", dslot_p[i+2+j], dslot_timer.procs[dslot_p[i+2+j]][0].wait);
								}
								
								if(dslot_map1[dslot_p[i+2+j]][dslot_p[i]] != 2)  
								{
									EPT(stderr, "proc3_2: hop1 = 2 \n");
									/* 将未占用或欲占用的时隙标记为已占用 */
									dslot_map1[dslot_p[i+2+j]][dslot_p[i]] = 2;
									dslot_map1[dslot_p[i+2+j]][0]++;
									dslot_map1[0][dslot_p[i]]++;
							...
							{
								EPT(stderr, "proc3_2: hop2 node = %d,dslot = %d\n", dslot_p[i], dslot_p[i+2+j]);
								/* 更新对应定时器 */
								sprintf(dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].name, "dtimer_D%dN%d", dslot_p[i+2+j], dslot_p[i]);
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period = (T4 * 1000)/NL_TINTVL_US;
								dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].wait = dslot_timer.procs[dslot_p[i+2+j]][dslot_p[i]].period;
...
void *hm_bs_life_thread(void *i_p)
{
	...
	struct timeval tv;
	tv.tv_sec = 4;      /* 有待调整 */ 
	tv.tv_usec = 96000;  /* 退网的时间严格按照两个超帧的时间来计算，确保满足某些条件时某些节点已经退网 11.15 */
...
void hm_LowMAC_slot_proc(U8 netnum)
{
	...
	*/
	...
	{
		if(count == dslot_map1[0][localID])
			break;
		if(dslot_map1[i][localID] == 2)
		{
			count++;
			LM_neighbor_map.dynamic_slot[(i-1)/8] = LM_neighbor_map.dynamic_slot[(i-1)/8] | (0b10000000 >> ((i-1)%8));
		}
	}
	
	/* 本节点固定时隙占用算法，由两跳内节点占用的广播时隙来决定 */
    for(i=0; i<MAX_CFS_PSF; i++)
    {
				...
				{					
					if(neighbor_map[net_i]->BS[i][j].BS_flag)
					{
						...
						if(neighbor_map[net_i]->BS[i][j].hop != 2)
						{
							...
							if(dslot_map1[0][j] != 0)
							{
								EPT(stderr, "sf_ls_send: node = %d\n", j);
								sf_dslot_map[sf_dslot_count++] = j;  //记录节点
								sf_dslot_map[sf_dslot_count++] = dslot_map1[0][j];  //记录节点占用几个动态时隙
								EPT(stderr, "sf_ls_send: num = %d\n", dslot_map1[0][j]);
								sf_dslot_num = 0;
								for(k = 1; k <= MAX_DSLS_PCF; k++)
								{
									if(dslot_map1[k][j] == 2)  //查找到节点占用的动态时隙
									{
										EPT(stderr, "sf_ls_send: dslot = %d\n", k);
										sf_dslot_map[sf_dslot_count++] = k;  //记录节点占用的动态时隙
										sf_dslot_num++;
										if(sf_dslot_num == dslot_map1[0][j])
											break;
