...
U8    HSN[6] = {0,0,0,0,0,0};  /* 记录每个队列的当前的HSN值 11.11 */
U8    LSN = 0;
...
void *hm_sendto_lm_thread(void *arg)
{
	...
	pthread_t busy_queue = -1; /* 测试动态时隙，建立业务队列下发线程 16.12.13 */
	...
	while(0 == stop1)
	{
		...
		{
			//usleep(4000);
			if(link_queue[4].front != link_queue[4].rear)     /* 状态改变帧，转发表，时隙表，BB，RF */
			{			
			...
			if(link_queue[5].front != link_queue[5].rear)     /* 勤务帧数据 */
			{			
...
#if 0
			if(link_queue[0].temp->next != NULL)     /* 路由协议，MAC协议数据(不包括勤务帧) */
			{			
				pthread_mutex_lock(&mutex_queue[0]);
				hm_queue_delete_flow_ctrl(link_queue, 0, &package);   /* package.len 是 package.data 里有效数据的长度 */
				pthread_mutex_unlock(&mutex_queue[0]);

				ht_msg.mtype = 1;  //必须大于0!
				ht_msg.node = *(U8 *)arg;
				memcpy(ht_msg.data, &package, package.len);
				len = package.len + 1;
				rcnt = msgsnd(ht_qid, &ht_msg, len, 0);
...
#endif
			if(link_queue[0].front != link_queue[0].rear)     /* 路由协议，MAC协议数据(不包括勤务帧) */
			{			
				pthread_mutex_lock(&mutex_queue[0]);
...
int hm_nARQ(lm_packet_t *package)
{
	...
	for(i=0; i<QUEUE_NUM-3; i++)
	{
		if(queue[i].q_flag == 0 && queue[i].HSN_flag == 0)      /* 无操作 */
			continue;
		if(queue[i].HSN < 0 || queue[i].HSN > 15)  /* HSN校验，防止LowMAC出错 */
		{	
