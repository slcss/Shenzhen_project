...
U8 BS[MAX_CFS_PSF][MAX_NODE_CNT+1];   /* 记录广播时隙占用情况，第一个数字对应时隙号，每个时隙可以被多个节点占用，第二个数字对应节点号,BS[][0]记录此时隙节点数 11.03 */
U8 node_BS_flag[MAX_NODE_CNT+1];     /* 记录每个节点是否正在占用广播时隙 [1]对应1号节点 [0]空闲 0对应没占用*/
U8 node_BS_backup[MAX_NODE_CNT+1];   /* 记录每个节点占用的广播时隙 [1]对应1号节点 [0]空闲 */
...
void ht_timer_proc(int signo)
{
	...
	data1.Lsn = 0;	
	msg_test.mtype = 1;  //必须大于0!!!!!!!!
	memcpy(msg_test.data, &data1, sizeof(data1));
		...
		if(BS_top[i][0])
		{
			for(j = 1; j <= MAX_NODE_CNT; j++)
			{
				...
				{					
					...
					//else						
						//EPT(stderr, "timer_proc: msgsnd() write msg at qid %d of node %d\n", rqs[j-1], j);
	...
	/* 非广播时隙达到，即业务时隙达到 16.12.12 */
#if 0	
	else
	{
		/* 每个业务时隙到达，先向在网的所有节点发送时隙到达通知帧 16.12.09 */
		for(i = 1; i <= MAX_NODE_CNT; i++)
		{
			if(node_BS_flag[i])
			{			
				rval = msgsnd(rqs[i-1], (void *)&msg_test, data1.len+1, 0);
				if ( rval < 0 )
				{
					EPT(fp, "timer_proc: msgsnd() write msg failed,errno=%d[%s]\n", errno, strerror(errno));
				}
				//EPT(stderr, "LowMAC send notice to node %d\n", i);			
			}
		}	
	}
#endif	
