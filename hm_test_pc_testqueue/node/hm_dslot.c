...
U8 dslot_order_num = 0;	//动态时隙每复帧需预约的时隙个数
U8 dslot_drop_num = 0;    //动态时隙每复帧需释放的时隙个数
...
U8 dslot_map1[MAX_DSLS_PCF+1][MAX_CFS_PSF+1];  //纵向代表D1-D55共55个动态时隙，横向代表1-32个节点，dslot_map[0][i]表示i节点占用的时隙总数，D[i][0]表示占用i时隙节点的总个数，值为0表示时隙未占用，1表示预占用，2表示已占用
...
void *hm_dslot_test_thread(void *arg)
{
	...
	while(0 == stop)
	{
		...
		N1 = sum/10;
		N2 = hm_get_slot_num();
		q_len = link_queue[0].real_l + link_queue[1].real_l + link_queue[2].real_l + link_queue[3].real_l + link_queue[5].real_l;
		avg_new = RED(avg_old, q_len);
		avg_old = avg_new;
